    ...
    var data = {
        ...
        verifySizeTruncation: function verifySizeTruncation(db, myCollName, options) {
            ...
            if (isMongod(db) && isMMAPv1(db)) {
                ...
                assertWhenOwnDB.eq(db[myCollName].find().itcount(), 1);

                // Insert another large document and verify that at least one truncation has
                // occurred. There should be 1 document in the collection and it should always be 
                ...
                // should be 4 documents in the collection, and they should be the most recently
                // inserted documents unless max documents is less than 4.
            ...
            // Truncation of capped collections is generally unreliable. Instead of relying
            // on its behavior, we test its occurrence. We set a reasonable threshold of documents 
            // to insert before a user might expect truncation to occur and verify truncation 
            // occurred for the right documents
